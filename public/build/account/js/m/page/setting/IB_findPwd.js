...
require(['js/m/lib/IB_fn', 'js/m/lib/IB_zepto' ,'js/m/lib/IB_fastclick',"js/m/lib/IB_reg"], function (fn,$,FastClick,reg) {
            ...
            if(!reg.pwd_reg.test(pwd)){
                ...
                findUtil.ajaxRequest("/api/pwd/reset",{
                    ...
                    if(data.status == 10000){
                        ...
                        fn.storage("user_rem_m",account);
                        setTimeout(function(){
                            location.href = global.host.m+"/login"
                        },1500);
                    }else if(data.status==10002){
                        findUtil.showPopTips("请输入有效的注册账号");
                    }else if(data.status == 10006){
