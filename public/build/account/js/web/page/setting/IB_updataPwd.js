        ...
        if(global.type == 1){
            ...
            $(".btn-change-email").click(function(){
                if($(this).val() == "更换邮箱号码"){
                    ...
                    newEmail = $.trim($(".setting-email").val());
                    if((regex.email.test(newEmail)&& email_validated == "false")||(email_validated == "true" && regex.mail.test(newEmail) && newEmail != email)){
            ...
            $(".btn-change-phone").click(function(){
                if($(this).val() == "更换手机号码"){
                    ...
                    if(regex.phone.test(newPhone)&& newPhone!=phone){
                        if( can_request){
                            can_request = false;
                            $.cookie("codeTime_phone",60,60);
                            showTime("codeTime_phone",".remind-resend-code",".btn-change-phone");
                            $.ajax({
                                type:"POST",
                                url:"/api/sendCode",
            ...
            $(".send-code").click(function(){
                ...
                if(regex.code.test(code)){
                        ...
                        $.ajax({
                            ...
                            success:function(data){
                                ...
                                if(data.status == 10000){
                                    ...
                                    $(".code-area").find(".input-tips").remove();
                                    $(".code-area").append("<span class='input-tips'><span class='icon-prompt'></span>验证码错误或过期</span>");
                                }else if(data.status == 10012){
                                    $(".code-area").find(".input-tips").remove();
                                    $(".code-area").append("<span class='input-tips'><span class='icon-prompt'></span>验证码错误</span>");
                                }
                            }
                        });
                    }
                    ...
                    $(".code-area").find(".input-tips").remove();
                    $(".code-area").append("<span class='input-tips'><span class='icon-prompt'></span>验证码格式错误</span>");
                }
            });
