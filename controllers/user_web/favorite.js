...
exports.listPage = function (req, res) {
    var page = req.query.page > 1 ? req.query.page : 1, type = req.query.type, uid = res.locals.uid, now_time = +new Date, timestamp = (page > 1 && req.cookies['intern_list_ts']) ? req.cookies['intern_list_ts'] : now_time;
    ...
    async.parallel([
        ...
        if (!ids || !ids.length) {
            return res.render("favorite/" + type + "List", {
                favorites: [],
                pages: pages,
                page: page,
                count: count
        ...
        proxy[type].getListByIds(ids, function (e, favorites) {
            ...
            res.render('favorite/' + type + 'List', data);
        });
    });
};
