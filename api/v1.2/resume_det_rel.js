...
exports.resumeDelivery = function (req, res) {
    var option = req.body.option || {}, uid = req.auth.uid, resumeone = req.insert_data.resumes[0];
    if (!option.det_id) {
...
exports.list = function (req, res) {
    var now_time = +new Date, page = req.query.page > 1 ? req.query.page : 1, timestamp = (page > 1 && req.headers['intern_list_ts']) ? req.headers['intern_list_ts'] : now_time, channel_type = req.query.ct || 0;
    proxy.resume_det_rel.list(req.auth.uid, page, channel_type, timestamp, function (err, data) {
        if (err) {
            logger.error(err);
            return res.json(resp_status_builder.build(10005, "服务器错误"));
        }
